...
def main():
    ...
    def predict(message, history):
        ...
        if prompt_template:
            ...
            for item in history:
                if isinstance(item, dict):
                    ...
                    if isinstance(user_msg, (list, tuple)): user_msg = str(user_msg[0])
                    if isinstance(assistant_msg, (list, tuple)): assistant_msg = str(assistant_msg[0])
        ...
        for new_token in streamer:
            stop = False
            pos = new_token.find(stop_str)
            if pos != -1:
                new_token = new_token[:pos]
                stop = True
